...
public final class IconPackPreviewView extends View {
    ...
    @Override protected void onDraw(Canvas canvas){
        super.onDraw(canvas); float w=getWidth(),h=getHeight(),pad=Math.min(w,h)*.08f;
        ...
        for(int i=0;i<MARKS.length;i++){
            int col=i%5,row=i/5; float x=pad+col*((w-pad*2)/4f),y=h*.28f+row*h*.38f; float size=Math.min(w,h)*.145f;
