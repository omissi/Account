...
public class MainActivity extends Activity {
    ...
    private void showOnboarding(){
        ...
        TextView privacy=label("No account · No first-launch permissions\nلا يحتاج إلى تسجيل",12,MUTED,false);privacy.setGravity(Gravity.CENTER);box.addView(privacy,matchWrap(dp(18),0));setContentView(root);
    }
    ...
    private void render(String destination){
        ...
        if(!destination.equals("detail")&&!destination.equals("editor"))addBottomNavigation();setContentView(root);
    }
    ...
    private LinearLayout brandHeader(){LinearLayout h=row();h.setGravity(Gravity.CENTER_VERTICAL);ImageView icon=new ImageView(this);icon.setImageResource(R.mipmap.ic_launcher);icon.setContentDescription("Vitra");h.addView(icon,new LinearLayout.LayoutParams(dp(48),dp(48)));h.addView(label("VITRA",24,TEXT,true),new LinearLayout.LayoutParams(0,-2,1));TextView beta=label("BETA",10,accent,true);beta.setGravity(Gravity.CENTER);beta.setBackground(glass(18,.9f,accent));h.addView(beta,new LinearLayout.LayoutParams(dp(64),dp(34)));return h;}
